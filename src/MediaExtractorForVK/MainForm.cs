...
internal sealed class MainForm : Form
{
    ...
    private void SelectSimilarCandidates()
    {
        ...
        var answer = MessageBox.Show(
            ...
            "Приложение отметит кандидатов, которые не выбраны как рекомендуемый экземпляр. " +
    ...
    private async Task ExportReportAsync()
    {
        ...
        using var dialog = new SaveFileDialog
        {
            ...
            DefaultExt = "csv",
            AddExtension = true,
            FileName = $"MediaExtractorForVK-report-{DateTime.Now:yyyyMMdd-HHmm}.csv",
    ...
    private string? FindQuarantineFolder()
    {
        ...
        foreach (var root in GetRootPaths(requireExisting: true))
        {
            try
            {
                var latest = Directory.EnumerateDirectories(root, ".MediaExtractorForVK_Quarantine_*")
