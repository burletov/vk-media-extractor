...
{
    public const string DestinationFolderName = "MediaTidy - Категории";
    private const string LegacyDestinationFolderName = "PhotoCleaner - Категории";
    ...
    private static void RemoveEmptyCategoryDirectories(IEnumerable<CategoryMove> moves)
    {
        ...
        foreach (var directory in directories)
        {
            ...
            while (Directory.Exists(current) &&
                   ...
                   !Path.GetFileName(current)
                       .Equals(LegacyDestinationFolderName, StringComparison.OrdinalIgnoreCase) &&
    ...
    {
        var current = Path.Combine(rootPath, DestinationFolderName);
        if (Directory.Exists(current))
        {
            return current;
        }

        var legacy = Path.Combine(rootPath, LegacyDestinationFolderName);
