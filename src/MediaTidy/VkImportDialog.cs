...
internal sealed class VkImportDialog : Form
{
        ...
        bool darkTheme)
    {
        ...
        var accessWarning = new Label
        {
            ...
            Tag = "warning",
            Text =
                "Важно: доступ к messages.* выдаётся не всем приложениям VK. Не закрывайте целевую папку и не удаляйте .part или .MediaTidy_VK_Import_*.json: " +
    ...
    private void OpenVkHost()
    {
        var answer = MessageBox.Show(
            ...
            "Проверяйте название приложения и запрашиваемые права. MediaTidy не получает и не сохраняет токен. " +
